...
class RandomResizedCropAndInterpolationWithTwoPicTwoPair:
    """Crop the given PIL Image to random size and aspect ratio with random interpolation.
    ...
    def __call__(self, img):
        ...
        if isinstance(self.interpolation, (tuple, list)):
            ...
            interpolation = self.interpolation
        if self.second_size is None:
            return F.resized_crop(img1, i, j, h, w, self.size, interpolation), \
                   F.resized_crop(img2, i, j, h, w, self.size, interpolation)
        else:
            return F.resized_crop(img1, i, j, h, w, self.size, interpolation), \
                   F.resized_crop(img2, i, j, h, w, self.size, interpolation), \
                   F.resized_crop(img1, i, j, h, w, self.second_size, self.second_interpolation),\
                   F.resized_crop(img2, i, j, h, w, self.second_size, self.second_interpolation)
...
class RandomResizedCropAndInterpolationWithTwoPicAndCoord:
    """Crop the given PIL Image to random size and aspect ratio with random interpolation.
...
class RandomResizedCropAndInterpolationWithTwoPicEVA:
    """Crop the given PIL Image to random size and aspect ratio with random interpolation.
