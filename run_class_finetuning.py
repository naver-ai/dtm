...
from engines.engine_for_finetuning import train_one_epoch, evaluate
...
def get_args():
    ...
    # init types
    parser.add_argument('--init_both_rpb', action='store_true')
    parser.add_argument('--init_rpb_only', action='store_true')
    parser.add_argument('--init_srpb_only', action='store_true')
    parser.set_defaults(init_srpb_only=True)
...
def main(args, ds_init):
        ...
        if args.init_both_rpb:
            if getattr(model, 'use_rel_pos_bias', False) and "rel_pos_bias.relative_position_bias_table" in checkpoint_model:
                ...
                for i in range(num_layers):
                    checkpoint_model["blocks.%d.attn.relative_position_bias_table" % i] = checkpoint_model["blocks.%d.attn.relative_position_bias_table" % i] + rel_pos_bias.clone()

                checkpoint_model.pop("rel_pos_bias.relative_position_bias_table")
        elif args.init_rpb_only:
            checkpoint_model.pop("rel_pos_bias.relative_position_bias_table")
            pass
        elif args.init_srpb_only:
            if getattr(model, 'use_rel_pos_bias', False) and "rel_pos_bias.relative_position_bias_table" in checkpoint_model:
                print("Expand the shared relative position embedding to each transformer block. ")
                num_layers = model.get_num_layers()
                rel_pos_bias = checkpoint_model["rel_pos_bias.relative_position_bias_table"]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            log_stats = {**{args.log_name + '/' + f'train_{k}': v for k, v in train_stats.items()},
                         **{args.log_name + '/' + f'test_{k}': v for k, v in test_stats.items()},
                         'epoch': epoch}
        else:
            log_stats = {**{args.log_name + '/' + f'train_{k}': v for k, v in train_stats.items()},
        ...
        if args.output_dir and utils.is_main_process():
            ...
            if data_loader_val is not None:
                saver.save(step=epoch, num_steps=args.epochs, state=save_dict,
                           summary={'accuracy': test_stats["acc1"], 'epoch': '%d/%d' % (epoch + 1, args.epochs),
