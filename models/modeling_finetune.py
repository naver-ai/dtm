...
class VisionTransformer(nn.Module):
    ...
    def forward_tome(self, x, matching_r, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)
        batch_size, seq_len, _ = x.size()
        ...
        for blk in self.blocks:
            ...
            metric = x
            # Apply ToMe here
            merge, _ = bipartite_soft_matching(metric, matching_r, True, False)
            source = merge_source(merge, x, source)
            x, matching_size = merge_wavg(merge, x, matching_size)
        ...
        if self.fc_norm is not None:
            ...
            t = x[:, 1:, :]
            if return_patch_tokens:
                return self.head(self.fc_norm(t)), source
            else:
...
@register_model
def beit_tiny_patch16_224(pretrained=False, **kwargs):
